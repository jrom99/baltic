...
############ arguments
samogitia = argparse.ArgumentParser(
    description="samogitia.py analyses trees drawn from the posterior distribution by BEAST.\n"
)

samogitia.add_argument(
    "-b",
    "--burnin",
    default=0,
    type=int,
    help="Number of states to remove as burnin (default 0).\n",
)
samogitia.add_argument(
    "-nc",
    "--nocalibration",
    default=True,
    action="store_false",
    help="Use flag to prevent calibration of trees into absolute time (default True). Should be used if tip names do not contain information about *when* each sequence was collected.\n",
)
samogitia.add_argument(
    "-t",
    "--treefile",
    type=open,
    required=True,
    help="File with trees sampled from the posterior distribution (usually with suffix .trees).\n",
)
samogitia.add_argument(
    "-a",
    "--analyses",
    ...
    required=True,
    nargs="+",
    help="Analysis to be performed, can be a list separated by spaces.\n",
)
samogitia.add_argument(
    "-o",
    "--output",
    type=argparse.FileType("w"),
    default="samogitia.out.txt",
    help="Output file name (default samogitia.out.txt).\n",
)
samogitia.add_argument(
    "-s",
    "--states",
    type=str,
    default="0-inf",
    help="Define range of states for analysis.\n",
)
samogitia.add_argument(
    "-df",
    "--date_format",
    type=str,
    default="%Y-%m-%d",
    help="Define date format encoded in tips (default '%%Y-%%m-%%d').\n",
)
samogitia.add_argument(
    "-tf",
    "--tip_format",
    type=str,
    default="\|([0-9]+)\-*([0-9]+)*\-*([0-9]+)*$",
    help="Define regex for capturing dates encoded in tips (default '\|([0-9]+)\-*([0-9]+)*\-*([0-9]+)*$'.\n",
)

args = vars(samogitia.parse_args())
burnin, treefile, analyses, outfile, calibration, states, dformat, tformat = (
    args["burnin"],
    args["treefile"],
    args["analyses"],
    args["output"],
...
progress_update = (
    Ntrees / barLength
)  ## update progress bar every time a tick has to be added
...
available_analyses = [
    ...
    "transitions",
    "subtrees",
]  ## analysis names that are possible
...
for queued_analysis in analyses:  ## for each queued analysis check if samogitia can do anything about them (i.e. whether they're known analysis types)
    assert queued_analysis in available_analyses, (
        "%s is not a known analysis type\n\nAvailable analysis types are: \n* %s\n"
...
for line in treefile:  ## iterate through each line
    ...
    if cerberus is not None:  ## tree identified
        ...
        if treecount == 0:  ## At tree state 0 insert header into output file
            ...
            if lower == 0 and upper == float(
                "inf"
            ):  ## only add a header if not doing a chunk
                    ...
                    for k in ll.Objects:  ## iterate over branches
                        if isinstance(k, bt.leaf):  ## only interested in tips
                            if "A" in k.name:  ## if name of tip satisfies condition
                                tmrcas["A"].append(
                                    k.numName
                                )  ## add the tip's numName to tmrca list - tips will be used to ID the common ancestor
        ...
        #################################################################
        if (
            int(cerberus.group(1)) >= burnin and lower <= int(cerberus.group(1)) < upper
            ...
            if "RC" in analyses:  ## 'RC' was queued as an analysis
                ...
                outfile.write(
                    "\t%s\t%s\t%s\t%s\t%s" % (tNs, tSs, tuNs, tuSs, dNdS)
            ...
            if "tmrcas" in analyses:
                ...
                nodes = {
                    x: None for x in tmrcas.keys()
                }  ## each TMRCA will correspond to a single object
                score = {
                    x: len(ll.Objects) + 1 for x in tmrcas.keys()
                }  ## this will be used to score the common ancestor candidate
                for required in tmrcas.keys():  ## iterate over TMRCAs
                    searchNodes = sorted(
                        [
                            nd
                            for nd in ll.Objects
                            if nd.branchType == "node"
                            and len(nd.leaves) >= len(tmrcas[required])
                        ],
                        key=lambda n: len(n.leaves),
                    )  ## common ancestor candidates must have at least as many descendants as the list of search tips
                    for k in searchNodes:  ## iterate over candidates
                        common, queryLeft, targetLeft = overlap(
                            k.leaves, tmrcas[required]
                        )  ## find how many query tips exist as descendants of candidate nodes
                        if (
                            len(targetLeft) == 0 and len(queryLeft) <= score[required]
                        ):  ## all of query tips must be descended from common ancestor, every extra descendant of common ancestor not in query contributes to a score
                            nodes[required] = (
                                k  ## if score improved - assign new common ancestor
                            )
                            score[required] = len(
                                queryLeft
                            )  ## score is extra descendants not in the list of known tips

                outTMRCA = [
                    "%.6f" % (nodes[n].absoluteTime) for n in sorted(nodes.keys())
                ]  ## fetch absoluteTime of each identified common ancestor
            ...
            if "Sharp" in analyses:
                ...
                assert len(analyses) == 1, (
                    "More that one analysis queued in addition to Sharp, which is inadvisable"
                )
                outSharp = []
                for k in ll.Objects:
            ...
            if "transitions" in analyses:
                ...
                assert len(analyses) == 1, (
                    "More that one analysis queued in addition to transitions, which is inadvisable"
                )
                outTransitions = []
                for k in ll.Objects:
                    if k.traits.has_key("location.states") and k.parent.traits.has_key(
                        "location.states"
                    ):
                        cur_value = k.traits["location.states"]
                        par_value = k.parent.traits["location.states"]
                        if cur_value != par_value:
                            outTransitions.append(
                                "{1,%s,%s,%s}"
                                % (
                                    ll.treeHeight - k.height - 0.5 * k.length,
                                    par_value,
                                    cur_value,
                                )
                            )
                outfile.write(
                    "\t%d\t%s" % (len(outTransitions), "\t".join(outTransitions))
                )
            ###################################################
            if "subtrees" in analyses:
                traitName = "location.states"
                assert [k.traits.has_key(traitName) for k in ll.Objects].count(
                    True
                ) > 0, 'No branches have the trait "%s"' % (traitName)
                for k in ll.Objects:
                    if (
                        k.traits.has_key(traitName)
                        and k.parent.traits.has_key(traitName)
                        and k.parent.index != "Root"
                        and k.traits[traitName] != k.parent.traits[traitName]
                        and k.traits[traitName] == "human"
                    ):
                        ...
                        if isinstance(k, bt.leaf):  ## if dealing with a leaf - proceed
                            ...
                            if [ch.traits[traitName] for ch in k.children].count(
                                kloc
                            ) > 0:
                                proceed = True
                        ...
                        if (
                            ...
                            subtree_leaves = [
                                x.name for x in subtree if isinstance(x, bt.leaf)
                            ]

                            if len(subtree_leaves) > 0:
                                mostRecentTip = max(
                                    [
                                        bt.decimalDate(x.strip("'").split("|")[-1])
                                ...
                                while (
                                    sum(
                                        [
                                            ...
                                            for nd in subtree
                                            if isinstance(nd, bt.node)
                                            and nd.index != "Root"
                                        ]
                                    )
                                    > 0
                                ):  ## keep removing nodes as long as there are nodes with children that are not entirely within subtree
                                    for nd in sorted(
                                        [q for q in subtree if isinstance(q, bt.node)],
                                        key=lambda x: (
                                            sum(
                                                [
                                                    1 if ch in subtree else 0
                                    ...
                                    ):  ## iterate over nodes in subtree, starting with ones that have fewest valid children and are more recent
                                        child_status = [
                                            1 if ch in subtree else 0
                                            for ch in nd.children
                                        ]  ## check how many children of current node are under the right trait value

                                        if (
                                            sum(child_status) < 2 and nd.index != "Root"
                                        ):  ## if less than 2 children in subtree (i.e. not all children are under the same trait state)
                                            ...
                                            grand_parent.children.remove(
                                                nd
                                            )  ## remove node from its parent's children

                                            if (
                                                sum(child_status) == 0
                                            ):  ## node has no valid children - current grandparent will be removed on next iteration, since it will only have one child
                                                pass
                                            else:  ## at least one child is still valid - reconnect the one valid child to grandparent
                                                child = nd.children[
                                                    child_status.index(1)
                                                ]  ## identify the valid child
                                                child.parent = grand_parent  ## child's parent is now its grandparent
                                                grand_parent.children.append(
                                                    child
                                                )  ## child is now child of grandparent
                                                child.length += nd.length  ## child's length now includes it's former parent's length
                                            subtree.remove(
                                                nd
                                            )  ## remove node from subtree
                                outfile.write(
                                    "\t{%s,%s,%s,%s,%d}"
                                    % (
                                        k.absoluteTime,
                                        mostRecentTip,
                                        k.parent.traits[traitName],
                                        k.traits[traitName],
                                        len(subtree_leaves),
                                    )
                                )
        ...
        if treecount == threshold:  ## tree passed progress bar threshold
            ...
            timeElapsed = float(
                divmod(timeTakenSoFar.total_seconds(), 60)[0]
                + (divmod(timeTakenSoFar.total_seconds(), 60)[1]) / float(60)
            )
            timeRate = float(
                divmod(timeTakenSoFar.total_seconds(), 60)[0] * 60
                + divmod(timeTakenSoFar.total_seconds(), 60)[1]
            ) / float(treecount + 1)  ## rate at which trees have been processed
            processingRate.append(timeRate)  ## remember rate
            ETA = (
                (
                    sum(processingRate)
                    / float(len(processingRate))
                    * (Ntrees - treecount)
                )
                / float(60)
                / float(60)
            )  ## estimate how long it'll take, given mean processing rate
            ...
            sys.stderr.write(
                "[%-30s] %4d%%  trees: %5d  elapsed: %5.2f%1s  ETA: %5.2fh (%6.1e s/tree)"
                % (
                    "=" * (excessiveTrees / progress_update),
                    treecount / float(Ntrees) * 100.0,
                    treecount,
                    reportElapsed,
                    reportUnit,
                    ETA,
